...
( cd $ERL_TOP \
    && ./configure \
        CC=clang \
        CXX=clang \
        LIBS="-Wl,-Bstatic -lncursesw -ltinfo -lcrypto -lssl -lstdc++ -Wl,-Bdynamic" \
        CFLAGS="-Os" \
...
rm -fr $ERL_TOP
find /usr/local -regex '/usr/local/lib/erlang/\(lib/\|erts-\).*/\(man\|doc\|obj\|c_src\|emacs\|info\|examples\)' | xargs rm -fr
...
apt list --installed > /usr/local/lib/erlang/versions.txt
